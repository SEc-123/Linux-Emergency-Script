#!/bin/bash
################################################################################
# Ubuntu Incident Response and Analysis Script
################################################################################
...
# ---------------------------
{
    echo "Ubuntu Incident Response Report"
    echo "Generated on: $(date)"
    print_line
    echo "[1] System Information"
    echo "Distribution: Ubuntu $(lsb_release -sr 2>/dev/null || echo "Unknown")"
...
# ---------------------------
{
    ...
    # 如果系统使用 systemd，可以用 journalctl -S
    ...
    # 分析 /var/log/syslog
    echo "Last 50 lines of /var/log/syslog (filtered by date range):"
    if [ -f /var/log/syslog ]; then
        ...
        ' /var/log/syslog | tail -n 50
    else
        echo "syslog not found."
    fi
    
    # 分析 /var/log/auth.log
    echo "Last 50 lines of /var/log/auth.log (filtered by date range):"
...
# ---------------------------
{
    ...
    echo "Failed Login Attempts (from /var/log/auth.log):"
    if [ -f /var/log/auth.log ]; then
        grep "Failed password" /var/log/auth.log | tail -n 20 2>/dev/null || echo "No failed logins found."
    else
        echo "auth.log not found."
    fi

    echo "Recently Added/Modified Users (useradd|usermod in /var/log/auth.log):"
    if [ -f /var/log/auth.log ]; then
        grep -E "useradd|usermod" /var/log/auth.log | tail -n 20 2>/dev/null || echo "No recent user changes found."
    else
        echo "auth.log not found."
    fi
